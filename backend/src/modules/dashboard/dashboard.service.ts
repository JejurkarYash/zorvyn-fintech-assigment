...
export async function getCategories(startDate?: Date, endDate?: Date) {
...
export async function fetchMonthlyTrends(year?: number) {
...
export function validateYear(yearParam: unknown): {
  valid: boolean;
  year?: number;
  error?: string;
} {
  if (!yearParam) return { valid: true };

  const parsed = parseInt(yearParam as string);
